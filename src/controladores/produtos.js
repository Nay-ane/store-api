...
const ERRO_PRODUTO_NAO_CADASTRADO = "Não foi possível cadastrar o produto";
const ERRO_ATUALIZACAO_PRODUTO = "Não foi possível atualizar o produto";
const ERRO_PRODUTO_NAO_ENCONTRADO =
    "Não foi possível encontrar o produto solicitado";
const ERRO_PRODUTO_NAO_EXCLUIDO =
    "Não foi possível excluir o produto solicitado";
...
const atualizarProdutos = async (req, res) => {
    ...
    try {
        ...
        if (produtoAtualizado.rowsCount === 0) {
            return res.status(400).json({ mensagem: ERRO_ATUALIZACAO_PRODUTO });
        }
...
const excluirProdutos = async (req, res) => {
    ...
    try {
        ...
        const produtoExcluido = await conexao.query(query, [
            usuarioId,
            idProduto,
        ]);
        ...
        if (produtoExcluido.rowCount === 0) {
            return res
                .status(400)
                .json({ mensagem: ERRO_PRODUTO_NAO_EXCLUIDO });
        }
